...
**/
typedef EmitAndSemanticDiagnosticsBuilderProgram = {
	/**
		Emits the next affected file's emit result (EmitResult and sourceFiles emitted) or returns undefined if iteration is complete
		The first of writeFile if provided, writeFile of BuilderProgramHost if provided, writeFile of compiler host
		in that order would be used to write the files
	**/
	function emitNextAffectedFile(?writeFile:WriteFileCallback, ?cancellationToken:CancellationToken, ?emitOnlyDtsFiles:Bool, ?customTransformers:CustomTransformers):AffectedFileResult<EmitResult>;
	/**
