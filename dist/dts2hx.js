...
var typescript_NodeBuilderFlags = require("typescript").NodeBuilderFlags;
var ConverterContext = $hx_exports["ConverterContext"] = function(inputModuleName,moduleSearchPath,compilerOptions,stdLibMap,options) {
	...
	var result = Typescript.resolveModuleName(inputModuleName,moduleSearchPath + "/.",compilerOptions,this.host);
	...
	if(this.inputModule.packageId != null) {
		...
		if(this.inputModule.packageId.name != inputModuleName) {
			var result = Typescript.resolveModuleName(this.packageName,moduleSearchPath + "/.",compilerOptions,this.host);
			if(result.resolvedModule == null) {
				var failedLookupLocations = Reflect.field(result,"failedLookupLocations");
	...
	this.program = Typescript.createProgram(inputSourcePaths,compilerOptions,this.host);
...
ConverterContext.isHaxeModuleSource = function(tc,symbol,access) {
	if(!((symbol.flags & (typescript_SymbolFlags.Type | typescript_SymbolFlags.ValueModule)) != 0 || tool_TsSymbolTools.isConstructorTypeVariableSymbol(tc,symbol))) {
		if((symbol.flags & (typescript_SymbolFlags.Function | typescript_SymbolFlags.Variable)) != 0) {
...
ConverterContext.requiresHxClass = function(tc,symbol) {
	if((symbol.flags & (typescript_SymbolFlags.Class | typescript_SymbolFlags.ValueModule | typescript_SymbolFlags.Value)) == 0) {
...
ConverterContext.requiresAdditionalStructureType = function(tc,symbol) {
	if((symbol.flags & typescript_SymbolFlags.Interface) != 0) {
...
ConverterContext.isGlobalField = function(tc,symbol,access) {
	if(access._hx_index == 2) {
		...
		if(_g.length == 1) {
			var _g1 = _g[0];
			if((symbol.flags & (typescript_SymbolFlags.Variable | typescript_SymbolFlags.Function)) != 0 && !tool_TsSymbolTools.isConstructorTypeVariableSymbol(tc,symbol)) {
				return true;
			} else {
				return false;
			}
...
ConverterContext.prototype = {
	...
	,generateHaxeModulesFromSymbol: function(symbol,access) {
		...
		var isValueModuleOnlySymbol = (symbol.flags & typescript_SymbolFlags.ValueModule) != 0 && (symbol.flags & typescript_SymbolFlags.Type) == 0 && !isConstructorTypeVariable;
		var fundamentalTypePath = this.haxeTypePathMap.getTypePath(symbol,access,false);
		if(!fundamentalTypePath.isExistingStdLibType) {
			var hxModule;
			if((symbol.flags & typescript_SymbolFlags.Enum) != 0) {
				var isCompileTimeEnum = (symbol.flags & typescript_SymbolFlags.ConstEnum) != 0;
				...
				while(_g1 < _g2.length) {
					var v = _g2[_g1];
					++_g1;
					if((v.flags & typescript_SymbolFlags.EnumMember) != 0) {
						_g.push(v);
					}
				}
				var enumMembers = _g;
			...
			} else if((symbol.flags & typescript_SymbolFlags.TypeAlias) != 0) {
				var _g = [];
				var _g1 = 0;
				...
				while(_g1 < _g2.length) {
					var v = _g2[_g1];
					++_g1;
					if(v.kind == typescript_SyntaxKind.TypeAliasDeclaration) {
						_g.push(v);
					}
				}
				...
				var forceAbstractKind = (symbol.flags & typescript_SymbolFlags.ValueModule) != 0 || isConstructorTypeVariable;
				...
				var classDeclaration = Lambda.find(symbol.declarations,function(d) {
					return d.kind == typescript_SyntaxKind.ClassDeclaration;
				});
				var declaration;
				...
				var callSignatures = this.tc.getSignaturesOfType(declaredType,typescript_SignatureKind.Call);
				...
				if(classSuperType != null) {
					...
					while(_g1 < _g2.length) {
						...
						if(classSuperMatch != null) {
							if((m[0].flags & typescript_SymbolFlags.Method) != 0) {
								var format = typescript_TypeFormatFlags.NoTruncation;
				...
				if(classDeclaration != null) {
					if(!Lambda.exists(fields,function(f) {
						...
						var inlobj_pos_min = 20415;
						var inlobj_pos_max = 20440;
						var inlobj_isExtern = false;
						var inlobj_kind = haxe_macro_TypeDefKind.TDClass(null,[],false,false,false);
						var inlobj_fields_0 = { name : "new", kind : haxe_macro_FieldType.FFun({ args : [], ret : null, expr : null, params : []}), pos : { file : "src/ConverterContext.hx", min : 20423, max : 20438}};
			...
			} else if((symbol.flags & typescript_SymbolFlags.Interface) != 0) {
			...
			if(isConstructorTypeVariable) {
				...
				if(constructorTypeDeclaration != null) {
					...
					var constructSignatures = this.tc.getSignaturesOfType(constructorType,typescript_SignatureKind.Construct);
					var callSignatures = this.tc.getSignaturesOfType(constructorType,typescript_SignatureKind.Call);
			...
			var _g1 = [];
			var _g11 = 0;
			var _g2 = tool_TsSymbolTools.getExports(symbol);
			while(_g11 < _g2.length) {
				var v = _g2[_g11];
				++_g11;
				if((v.flags & typescript_SymbolFlags.ClassMember) != 0 && tool_TsSymbolTools.isAccessibleField(v)) {
					_g1.push(v);
				}
			}
			var _g2 = _g1;
			...
			if((symbol.flags & typescript_SymbolFlags.Module) != 0) {
				var _g = [];
				var _g1 = 0;
				var _g2 = this.tc.getExportsOfModule(symbol);
				while(_g1 < _g2.length) {
					var v = _g2[_g1];
					++_g1;
					if((v.flags & typescript_SymbolFlags.ModuleMember) != 0 && (tool_TsSymbolTools.isAccessibleField(v) || (v.flags & typescript_SymbolFlags.Alias) != 0)) {
						_g.push(v);
					}
				}
				...
				while(_g < moduleMemberFields.length) {
					...
					if((moduleMember.flags & typescript_SymbolFlags.Alias) != 0) {
	...
	,createInterfaceModule: function(symbol,access,preferInterfaceStructure) {
		...
		var declaration = Lambda.find(symbol.declarations,function(d) {
			return d.kind == typescript_SyntaxKind.InterfaceDeclaration;
		});
		var declaredType = this.tc.getDeclaredTypeOfSymbol(symbol);
		var _g = [];
		var _g1 = 0;
	...
	,generateTypeFields: function(symbol,access,declaration,constructorSignatures,callSignatures,indexSignatures,classMembers,haxeClassName) {
		...
		if((symbol.flags & typescript_SymbolFlags.Function) != 0) {
			var tsType = this.getTsTypeOfField(symbol);
			var signatures = this.tc.getSignaturesOfType(this.tc.getNonNullableType(tsType),typescript_SignatureKind.Call);
			...
			var tmp;
			if((symbol.flags & typescript_SymbolFlags.Variable) != 0) {
				if(access._hx_index == 1) {
					var _g = access.moduleName;
					var _g = access.sourceFileSymbol;
			...
			if(tmp) {
				...
				case 0:
					...
					var inlobj_name = "";
					var inlobj_pos_file = "src/ConverterContext.hx";
					var inlobj_pos_min = 30186;
					var inlobj_pos_max = 30278;
					var inlobj_isExtern = false;
					var inlobj_kind = haxe_macro_TypeDefKind.TDClass(null,[],false,false,false);
					var inlobj_fields_0 = { name : "get_value", access : [haxe_macro_Access.AStatic,haxe_macro_Access.AInline], kind : haxe_macro_FieldType.FFun({ args : [], ret : type, expr : { expr : haxe_macro_ExprDef.EReturn({ expr : haxe_macro_ExprDef.ECast({ expr : haxe_macro_ExprDef.EConst(haxe_macro_Constant.CIdent(haxeClassName)), pos : { file : "src/ConverterContext.hx", min : 30255, max : 30270}},null), pos : { file : "src/ConverterContext.hx", min : 30248, max : 30270}}), pos : { file : "src/ConverterContext.hx", min : 30241, max : 30270}}, params : []}), pos : { file : "src/ConverterContext.hx", min : 30200, max : 30270}};
	...
	,isTypeStructureInHaxe: function(type,moduleSymbol,accessContext,enclosingDeclaration) {
		if((type.flags & typescript_TypeFlags.Object) != 0) {
			if(this.isHxAny(this.complexTypeFromTsType(type,moduleSymbol,accessContext,enclosingDeclaration))) {
				return false;
			}
			var objectType = type;
			var isAnonType = (objectType.objectFlags & typescript_ObjectFlags.Anonymous) != 0;
			var isInterface = type.symbol != null && (type.symbol.flags & typescript_SymbolFlags.Interface) != 0;
			var isValueModule = type.symbol != null && (type.symbol.flags & typescript_SymbolFlags.ValueModule) != 0;
	...
	,complexTypeFromTsType: function(type,moduleSymbol,accessContext,enclosingDeclaration,disallowAliasTarget,preferInterfaceStructure) {
		...
		if(type.aliasSymbol != null && disallowAliasTarget != type.aliasSymbol) {
			...
			var isAliasToMappedType = (type.flags & typescript_TypeFlags.Object) != 0 && (type.objectFlags & typescript_ObjectFlags.Mapped) != 0;
		...
		try {
			...
			} else if((type.flags & typescript_TypeFlags.Void) != 0) {
				complexType = SupportTypes.getUndefinedType(this);
			} else if((type.flags & typescript_TypeFlags.Enum) != 0) {
			...
			} else if((type.flags & typescript_TypeFlags.Union) != 0) {
				complexType = this.complexTypeFromUnionType(type,moduleSymbol,accessContext,enclosingDeclaration);
			} else if((type.flags & typescript_TypeFlags.Intersection) != 0) {
				complexType = this.complexTypeFromIntersectionType(type,moduleSymbol,accessContext,enclosingDeclaration);
			} else if((type.flags & typescript_TypeFlags.StringLiteral) != 0) {
				complexType = haxe_macro_ComplexType.TPath({ pack : [], name : "String", params : []});
			} else if((type.flags & typescript_TypeFlags.NumberLiteral) != 0) {
	...
	,complexTypeFromIntersectionType: function(intersectionType,moduleSymbol,accessContext,enclosingDeclaration) {
		...
		while(_g1 < _g2.length) {
			var v = _g2[_g1];
			++_g1;
			if((v.flags & typescript_TypeFlags.NonPrimitive) == 0) {
				_g.push(v);
			}
		}
		var types = _g;
	...
	,complexTypeFromObjectType: function(objectType,moduleSymbol,accessContext,preferInterfaceStructure,enclosingDeclaration) {
		if((objectType.objectFlags & typescript_ObjectFlags.Reference) != 0) {
		...
		} else if((objectType.objectFlags & typescript_ObjectFlags.ClassOrInterface) != 0) {
		...
		} else if((objectType.objectFlags & typescript_ObjectFlags.Anonymous) != 0) {
	...
	,complexTypeAnonFromTsType: function(tsType,moduleSymbol,accessContext,enclosingDeclaration) {
		var _gthis = this;
		if((tsType.flags & typescript_TypeFlags.TypeParameter) != 0) {
	...
	,functionFieldFromSignatures: function(fieldName,signatures,moduleSymbol,accessContext,enclosingDeclaration) {
		...
		while(_g < _g1) {
			...
			fun.expr = { expr : haxe_macro_ExprDef.EBlock([]), pos : { file : "src/ConverterContext.hx", min : 53673, max : 53675}};
	...
	,complexTypeFromTypeReference: function(typeReference,moduleSymbol,accessContext,preferInterfaceStructure,enclosingDeclaration) {
		var _gthis = this;
		if((typeReference.objectFlags & typescript_ObjectFlags.ClassOrInterface) != 0) {
		...
		} else if((typeReference.target.objectFlags & typescript_ObjectFlags.Tuple) != 0) {
	...
	,complexTypeFromGenericType: function(genericType,moduleSymbol,accessContext,preferInterfaceStructure,enclosingDeclaration) {
		if((genericType.objectFlags & typescript_ObjectFlags.Tuple) != 0) {
	...
	,getTsTypeOfField: function(symbol) {
		if(symbol.valueDeclaration != null) {
			...
			if((symbol.flags & typescript_SymbolFlags.Prototype) != 0 && parentSymbol != null) {
	...
	,fieldFromSymbol: function(nativeFieldName,symbol,moduleSymbol,accessContext,enclosingDeclaration) {
		...
		var isOptional = (symbol.flags & typescript_SymbolFlags.Optional) != 0;
		var baseDeclaration;
		if(symbol.valueDeclaration != null) {
			baseDeclaration = symbol.valueDeclaration;
		} else if((symbol.flags & typescript_SymbolFlags.Prototype) == 0) {
		...
		var kind;
		if((symbol.flags & (typescript_SymbolFlags.PropertyOrAccessor | typescript_SymbolFlags.Variable)) != 0) {
			var nullFreeTsType = this.tc.getNonNullableType(tsType);
			var isNullable = tool_TsTypeTools.isNullishUnion(tsType);
			var callSignatures = this.tc.getSignaturesOfType(nullFreeTsType,typescript_SignatureKind.Call);
			var constructSignatures = this.tc.getSignaturesOfType(nullFreeTsType,typescript_SignatureKind.Construct);
			var _g = [];
			var _g1 = 0;
			...
			if(callSignatures.length > 0 && constructSignatures.length == 0 && typeFields.length == 0) {
				...
				if(baseDeclaration != null) {
					...
					case typescript_SyntaxKind.PropertyDeclaration:case typescript_SyntaxKind.PropertySignature:case typescript_SyntaxKind.VariableDeclaration:
						break;
					case typescript_SyntaxKind.GetAccessor:case typescript_SyntaxKind.SetAccessor:
						break;
					default:
				...
				if((symbol.flags & typescript_SymbolFlags.GetAccessor) != 0 && (symbol.flags & typescript_SymbolFlags.SetAccessor) == 0) {
					var item = haxe_macro_Access.AFinal;
					if(hxAccessModifiers.indexOf(item) == -1) {
						hxAccessModifiers.push(item);
					}
		...
		} else if((symbol.flags & (typescript_SymbolFlags.Method | typescript_SymbolFlags.Function)) != 0) {
			if(baseDeclaration != null) {
				switch(baseDeclaration.kind) {
				case typescript_SyntaxKind.FunctionDeclaration:case typescript_SyntaxKind.MethodDeclaration:case typescript_SyntaxKind.MethodSignature:
					break;
				default:
					onError("Unhandled declaration kind <b>" + tool_TsSyntaxTools.getSyntaxKindName(baseDeclaration.kind) + "</>");
				}
			...
			var nullFreeTsType = this.tc.getNonNullableType(tsType);
			var signatures = this.tc.getSignaturesOfType(nullFreeTsType,typescript_SignatureKind.Call);
			kind = kindFromFunctionSignatures(signatures);
		} else if((symbol.flags & typescript_SymbolFlags.EnumMember) != 0) {
			var parent = tool_TsSymbolTools.getSymbolParent(symbol);
			var isConstEnum;
			if(parent != null) {
				isConstEnum = (parent.flags & typescript_SymbolFlags.ConstEnum) != 0;
			} else {
	...
	,functionFromSignature: function(signature,moduleSymbol,accessContext,enclosingDeclaration) {
		...
		if(signature.parameters != null) {
			...
			while(_g < _g1) {
				...
				var isOptional = parameterDeclaration != null && _gthis.tc.isOptionalParameter(s.valueDeclaration);
		...
		var hxRet = (tsRet.flags & typescript_TypeFlags.Void) != 0 ? haxe_macro_ComplexType.TPath({ pack : [], name : "Void", params : []}) : this.complexTypeFromTsType(tsRet,moduleSymbol,accessContext,signature.declaration);
	...
	,typeParamDeclFromTsTypeParameter: function(typeParameter,moduleSymbol,accessContext,enclosingDeclaration) {
		var typeParamNode = Lambda.find(typeParameter.symbol.declarations,function(d) {
			return d.kind == typescript_SyntaxKind.TypeParameter;
		});
	...
	,accessFromModifiers: function(modifiers,logSymbol) {
		...
		while(_g < _g1.length) {
			...
			case typescript_SyntaxKind.AbstractKeyword:
				Log.warn("`abstract` modifier not handled",null,logSymbol);
				break;
			case typescript_SyntaxKind.AsyncKeyword:
				Log.warn("`async` modifier not handled",null,logSymbol);
				break;
			case typescript_SyntaxKind.ConstKeyword:
				access.push(haxe_macro_Access.AFinal);
				break;
			case typescript_SyntaxKind.DeclareKeyword:
				break;
			case typescript_SyntaxKind.DefaultKeyword:
				break;
			case typescript_SyntaxKind.ExportKeyword:
				break;
			case typescript_SyntaxKind.PrivateKeyword:
				access.push(haxe_macro_Access.APrivate);
				break;
			case typescript_SyntaxKind.ProtectedKeyword:
				access.push(haxe_macro_Access.APrivate);
				break;
			case typescript_SyntaxKind.PublicKeyword:
				access.push(haxe_macro_Access.APublic);
				break;
			case typescript_SyntaxKind.ReadonlyKeyword:
				access.push(haxe_macro_Access.AFinal);
				break;
			case typescript_SyntaxKind.StaticKeyword:
...
HaxeTypePathMap.prototype = {
	getTypePath: function(symbol,accessContext,preferInterfaceStructure) {
		if((symbol.flags & typescript_SymbolFlags.Alias) != 0) {
				...
				while(_g5_current < _g5_length) {
					...
					if(matches1.length > 1) {
						...
						haxe_ds_ArraySort.sort(matches1,function(a,b) {
							var renameabilityA = (a.renameable ? 1 : 0) << 7 | (!a.isExistingStdLibType ? 1 : 0) << 6 | (a.access._hx_index == 3 ? 1 : 0) << 5 | ((a.symbol.flags & typescript_SymbolFlags.Class) == 0 ? 1 : 0) << 4 | ((a.symbol.flags & typescript_SymbolFlags.Enum) == 0 ? 1 : 0) << 3 | ((a.symbol.flags & typescript_SymbolFlags.Interface) == 0 ? 1 : 0) << 2 | ((a.symbol.flags & typescript_SymbolFlags.TypeAlias) == 0 ? 1 : 0) << 1 | ((a.symbol.flags & typescript_SymbolFlags.ValueModule) == 0 ? 1 : 0);
							var renameabilityB = (b.renameable ? 1 : 0) << 7 | (!b.isExistingStdLibType ? 1 : 0) << 6 | (b.access._hx_index == 3 ? 1 : 0) << 5 | ((b.symbol.flags & typescript_SymbolFlags.Class) == 0 ? 1 : 0) << 4 | ((b.symbol.flags & typescript_SymbolFlags.Enum) == 0 ? 1 : 0) << 3 | ((b.symbol.flags & typescript_SymbolFlags.Interface) == 0 ? 1 : 0) << 2 | ((b.symbol.flags & typescript_SymbolFlags.TypeAlias) == 0 ? 1 : 0) << 1 | ((b.symbol.flags & typescript_SymbolFlags.ValueModule) == 0 ? 1 : 0);
...
Log.diagnostics = function(message,array) {
	...
	while(_g < array.length) {
		...
		case typescript_DiagnosticCategory.Error:
			Log.error(message,null,null,null,diagnostic);
			break;
		case typescript_DiagnosticCategory.Message:
			Log.log(message,null,null,null,diagnostic);
			break;
		case typescript_DiagnosticCategory.Suggestion:
			Log.log(message,null,null,null,diagnostic);
			break;
		case typescript_DiagnosticCategory.Warning:
...
Main.main = function() {
	...
	defaultCompilerOptions.target = typescript_ScriptTarget.ES2015;
	defaultCompilerOptions.types = [];
	defaultCompilerOptions.moduleResolution = typescript_ModuleResolutionKind.NodeJs;
	...
	if(cliOptions.tsConfigFilePath != null) {
		var readResult = Typescript.readConfigFile(cliOptions.tsConfigFilePath,function(path) {
			return Typescript.sys.readFile(path,"utf8");
		});
		if(readResult.config != null) {
			var compilerOptionsObj = Reflect.field(readResult.config,"compilerOptions");
			var result = Typescript.convertCompilerOptionsFromJson(compilerOptionsObj,process.cwd(),cliOptions.tsConfigFilePath);
		...
		try {
			...
			while(_g < allDependencies.length) {
				...
				if(result.resolvedModule != null) {
					...
					case typescript_Extension.Js:case typescript_Extension.Json:case typescript_Extension.Jsx:case typescript_Extension.TsBuildInfo:
						break;
					case typescript_Extension.Dts:case typescript_Extension.Ts:case typescript_Extension.Tsx:
	...
	var _g = 0;
	var _g1 = Typescript.getAutomaticTypeDirectiveNames(compilerOptions,host);
	while(_g < _g1.length) {
		var moduleName = _g1[_g];
		++_g;
		moduleQueue.tryEnqueue(moduleName);
...
Main.generateReadme = function(inputModuleName,moduleSearchPath,converter,modulePackageJson,stdLibTypeMap) {
	...
	sections.push("\n\t\t\tGenerated from **" + typesModuleIdMarkdown + "** by **" + dts2hxRef + " " + Main.dts2hxPackageJson.version + "** using **TypeScript " + Typescript.version + "** with arguments:\n\n\t\t\t\tdts2hx " + process.argv.slice(2).join(" ") + "\n\t\t");
...
Main.printDoc = function(argHandler) {
	var s = "<b>dts2hx</b> <b>" + Main.dts2hxPackageJson.version + "</> <red>alpha</> using <b>TypeScript " + Typescript.version + "</>";
	if(s == null) {
		s = "";
	}
	Console.printFormatted(s + "\n",0);
	...
	while(_g < _g1.length) {
		...
		while(_g2 < _g3.length) {
			...
			if(tool_TsSymbolTools.isAccessibleField(symbol)) {
				...
				if(fieldType.symbol != null && (fieldType.symbol.flags & typescript_SymbolFlags.Enum) != 0) {
					...
					var _g4 = [];
					var _g11 = 0;
					var _g21 = tc.getExportsOfModule(enumTypeSymbol);
...
SupportTypes.getTupleType = function(ctx,elementTypes) {
	...
	if(existingModule == null) {
		...
		while(_g < _g1) {
			...
			var inlobj_pos_min = 1036;
			var inlobj_pos_max = 1236;
			var inlobj_isExtern = false;
			var inlobj_kind = haxe_macro_TypeDefKind.TDClass(null,[],false,false,false);
			var inlobj_fields = [{ name : name, access : [haxe_macro_Access.APublic], kind : haxe_macro_FieldType.FProp("get","set",type,null), pos : { file : "src/SupportTypes.hx", min : 1049, max : 1083}},{ name : get, access : [haxe_macro_Access.AInline], kind : haxe_macro_FieldType.FFun({ args : [], ret : type, expr : { expr : haxe_macro_ExprDef.EReturn({ expr : haxe_macro_ExprDef.ECast({ expr : haxe_macro_ExprDef.EArray({ expr : haxe_macro_ExprDef.EConst(haxe_macro_Constant.CIdent("this")), pos : { file : "src/SupportTypes.hx", min : 1131, max : 1135}},indexExpr), pos : { file : "src/SupportTypes.hx", min : 1131, max : 1147}},null), pos : { file : "src/SupportTypes.hx", min : 1126, max : 1147}}), pos : { file : "src/SupportTypes.hx", min : 1119, max : 1147}}, params : []}), pos : { file : "src/SupportTypes.hx", min : 1089, max : 1147}},{ name : set, access : [haxe_macro_Access.AInline], kind : haxe_macro_FieldType.FFun({ args : [{ name : "v", opt : false, type : type}], ret : type, expr : { expr : haxe_macro_ExprDef.EReturn({ expr : haxe_macro_ExprDef.ECast({ expr : haxe_macro_ExprDef.EBinop(haxe_macro_Binop.OpAssign,{ expr : haxe_macro_ExprDef.EArray({ expr : haxe_macro_ExprDef.EConst(haxe_macro_Constant.CIdent("this")), pos : { file : "src/SupportTypes.hx", min : 1204, max : 1208}},indexExpr), pos : { file : "src/SupportTypes.hx", min : 1204, max : 1220}},{ expr : haxe_macro_ExprDef.ECast({ expr : haxe_macro_ExprDef.EConst(haxe_macro_Constant.CIdent("v")), pos : { file : "src/SupportTypes.hx", min : 1228, max : 1229}},null), pos : { file : "src/SupportTypes.hx", min : 1223, max : 1229}}), pos : { file : "src/SupportTypes.hx", min : 1204, max : 1229}},null), pos : { file : "src/SupportTypes.hx", min : 1199, max : 1229}}), pos : { file : "src/SupportTypes.hx", min : 1192, max : 1229}}, params : []}), pos : { file : "src/SupportTypes.hx", min : 1154, max : 1229}}];
		...
		while(_g1 < _g2) {
			var i = _g1++;
			_g.push({ expr : haxe_macro_ExprDef.EConst(haxe_macro_Constant.CIdent("element" + i)), pos : { file : "src/SupportTypes.hx", min : 1433, max : 1446}});
		}
		var elementInitializerArray = _g;
		var inlobj_name = "";
		var inlobj_pos_file = "src/SupportTypes.hx";
		var inlobj_pos_min = 1474;
		var inlobj_pos_max = 1578;
		var inlobj_isExtern = false;
		var inlobj_kind = haxe_macro_TypeDefKind.TDClass(null,[],false,false,false);
		var inlobj_fields_0 = { name : "new", access : [haxe_macro_Access.APublic,haxe_macro_Access.AInline], kind : haxe_macro_FieldType.FFun({ args : [{ name : newParams, opt : false, type : null}], ret : null, expr : { expr : haxe_macro_ExprDef.EBlock([{ expr : haxe_macro_ExprDef.EBinop(haxe_macro_Binop.OpAssign,{ expr : haxe_macro_ExprDef.EConst(haxe_macro_Constant.CIdent("this")), pos : { file : "src/SupportTypes.hx", min : 1532, max : 1536}},{ expr : haxe_macro_ExprDef.EArrayDecl(elementInitializerArray), pos : { file : "src/SupportTypes.hx", min : 1541, max : 1566}}), pos : { file : "src/SupportTypes.hx", min : 1532, max : 1566}}]), pos : { file : "src/SupportTypes.hx", min : 1525, max : 1573}}, params : []}), pos : { file : "src/SupportTypes.hx", min : 1486, max : 1573}};
		fields.unshift(inlobj_fields_0);
...
SupportTypes.getUnionType = function(ctx,types) {
	...
	var _g1 = [];
	var e1 = $getIterator(_g);
	while(e1.hasNext()) {
		var e11 = e1.next();
		var x1 = $getIterator(e11);
		while(x1.hasNext()) {
			var x11 = x1.next();
			_g1.push(x11);
		}
	...
	if(existingModule == null) {
		...
		while(_g1 < _g2) {
			...
			var inlobj_name = "";
			var inlobj_pos_file = "src/SupportTypes.hx";
			var inlobj_pos_min = 3566;
			var inlobj_pos_max = 3694;
			var inlobj_isExtern = false;
			var inlobj_kind = haxe_macro_TypeDefKind.TDClass(null,[],false,false,false);
			var inlobj_fields = [{ name : varName, kind : haxe_macro_FieldType.FProp("get","never",typeParam,null), pos : { file : "src/SupportTypes.hx", min : 3580, max : 3617}},{ name : getterName, access : [haxe_macro_Access.APrivate,haxe_macro_Access.AInline], kind : haxe_macro_FieldType.FFun({ args : [], ret : typeParam, expr : { expr : haxe_macro_ExprDef.EReturn({ expr : haxe_macro_ExprDef.EConst(haxe_macro_Constant.CIdent("this")), pos : { file : "src/SupportTypes.hx", min : 3682, max : 3686}}), pos : { file : "src/SupportTypes.hx", min : 3675, max : 3686}}, params : []}), pos : { file : "src/SupportTypes.hx", min : 3624, max : 3686}}];
...
SymbolAccessMap.prototype = {
	...
	,symbolAccessAppendSymbol: function(access,symbol) {
		if(symbol.escapedName == typescript_InternalSymbolName.ExportEquals) {
		...
		while(_g < _g1) {
			...
			if((existingSymbol.flags & typescript_SymbolFlags.Alias) != 0 && this.tc.getAliasedSymbol(existingSymbol) == symbol) {
				switch(access._hx_index) {
				case 0:
...
tool_TsProgramTools.getGlobalScopeSymbolsInSourceFile = function(program,sourceFile) {
	...
	while(_g < moduleAugmentations.length) {
		...
		if(Typescript.isIdentifier(moduleAugmentation)) {
			...
			if(identSymbol != null && identSymbol.escapedName == typescript_InternalSymbolName.Global) {
...
tool_TsSymbolTools.getId = function(symbol) {
	return Typescript.getSymbolId(symbol) | 0;
};
tool_TsSymbolTools.isAccessibleField = function(symbol) {
	var isKnownSymbol = StringTools.startsWith(symbol.escapedName,"__@");
	var FieldSymbolFlags = typescript_SymbolFlags.Variable | typescript_SymbolFlags.Function | typescript_SymbolFlags.ClassMember;
...
tool_TsSymbolTools.isSourceFileSymbol = function(symbol) {
	if((symbol.flags & typescript_SymbolFlags.Module) != 0 && symbol.valueDeclaration != null) {
		return Typescript.isSourceFile(symbol.valueDeclaration);
	} else {
		return false;
	}
};
tool_TsSymbolTools.isConstructorTypeVariableSymbol = function(tc,symbol) {
	if((symbol.flags & typescript_SymbolFlags.Variable) != 0 && symbol.valueDeclaration != null) {
...
tool_TsSymbolTools.getClassExtendsType = function(tc,symbol) {
	var classDeclaration = symbol.valueDeclaration != null && Typescript.isClassDeclaration(symbol.valueDeclaration) ? symbol.valueDeclaration : null;
	if(classDeclaration != null && classDeclaration.heritageClauses != null) {
		var extendsClause = Lambda.find(classDeclaration.heritageClauses,function(h) {
			return h.token == typescript_SyntaxKind.ExtendsKeyword;
		});
...
tool_TsSymbolTools.walkDeclarationSymbols = function(tc,symbol,onSymbol,accessChain,depth) {
	...
	var ignoredSymbolFlags = typescript_SymbolFlags.ExportStar;
	if((symbol.flags & typescript_SymbolFlags.Module) != 0) {
	...
	if((symbol.flags & (typescript_SymbolFlags.Type | typescript_SymbolFlags.Variable | typescript_SymbolFlags.Function | typescript_SymbolFlags.Property)) != 0) {
		handled = true;
		onSymbol(symbol,accessChain);
	}
	if((symbol.flags & typescript_SymbolFlags.Module) != 0) {
		...
		while(_g1 < _g2.length) {
			var v = _g2[_g1];
			++_g1;
			if((v.flags & typescript_SymbolFlags.ModuleMember) != 0) {
				_g.push(v);
			}
		}
...
tool_TsSymbolTools.getConstructorSignatures = function(symbol,tc) {
	...
	while(_g1 < _g2.length) {
		var v = _g2[_g1];
		++_g1;
		if((v.flags & typescript_SymbolFlags.Constructor) != 0 && v.escapedName == typescript_InternalSymbolName.Constructor) {
			_g.push(v);
		}
	}
	var symbols = _g;
	...
	while(_g < _g1) {
		...
		while(_g3 < _g4.length) {
			var v = _g4[_g3];
			++_g3;
			if(Typescript.isConstructorDeclaration(v)) {
				_g2.push(v);
			}
		}
		result[i] = _g2;
...
tool_TsSymbolTools.getDeclarationTypeParameters = function(symbol) {
	...
	while(_g1 < _g2.length) {
		...
		var tmp;
		switch(v.kind) {
		case typescript_SyntaxKind.ClassDeclaration:case typescript_SyntaxKind.InterfaceDeclaration:case typescript_SyntaxKind.TypeAliasDeclaration:
			tmp = true;
			break;
		default:
			tmp = false;
	...
	while(_g < typeDeclarations.length) {
		...
		++_g;
		var declarationTypeParameters = Typescript.getEffectiveTypeParameterDeclarations(declaration);
...
tool_TsSyntaxTools.isVarConst = function(node) {
	return (Typescript.getCombinedNodeFlags(node) & typescript_NodeFlags.Const) != 0;
};
...
tool_TsTypeTools.isNullishUnion = function(type) {
	if(type.isUnion()) {
		...
		return Lambda.exists(unionType.types,function(t) {
			return (t.flags & (typescript_TypeFlags.Null | typescript_TypeFlags.Undefined)) != 0;
		});
	} else {
		return false;
	}
...
tool_TsTypeTools.isConstructorType = function(tc,type) {
	return tc.getSignaturesOfType(type,typescript_SignatureKind.Construct).length > 0;
};
tool_TsTypeTools.getIndexSignaturesOfType = function(tc,type) {
	var indexDeclarations = [];
	var numberInfo = tc.getIndexInfoOfType(type,typescript_IndexKind.Number);
	var stringInfo = tc.getIndexInfoOfType(type,typescript_IndexKind.String);
...
var typescript_DiagnosticCategory = require("typescript").DiagnosticCategory;
var typescript_Extension = require("typescript").Extension;
var typescript_IndexKind = require("typescript").IndexKind;
var typescript_InternalSymbolName = require("typescript").InternalSymbolName;
var typescript_ModuleResolutionKind = require("typescript").ModuleResolutionKind;
var typescript_NodeFlags = require("typescript").NodeFlags;
var typescript_ObjectFlags = require("typescript").ObjectFlags;
var typescript_ScriptTarget = require("typescript").ScriptTarget;
var typescript_SignatureKind = require("typescript").SignatureKind;
var typescript_SymbolFlags = require("typescript").SymbolFlags;
var typescript_SyntaxKind = require("typescript").SyntaxKind;
var typescript_TypeFlags = require("typescript").TypeFlags;
var typescript_TypeFormatFlags = require("typescript").TypeFormatFlags;
...
ConverterContext.defaultNodeBuilderFlags = typescript_NodeBuilderFlags.NoTruncation | typescript_NodeBuilderFlags.WriteArrayAsGenericType;
